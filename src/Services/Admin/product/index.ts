...
export const add_new_product = async (formData: any) => {
  ...
  export const get_product_by_category_name = async (categoryName:string) => {
    try {
      const res = await fetch(`/api/common/product/get-product-by-category-name?name=${encodeURIComponent(categoryName)}`, {
        method: 'GET',
      });
      ...
      console.log('Error in getting product by category name (service) =>', error);
