...
export default function ForgetPassword() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // Removed error toast on failure
  }
        ...
        <div className="w-full p-6 bg-white rounded-lg shadow md:mt-0 sm:max-w-md sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4 lg:mt-5 md:space-y-5">
            ...
            {loading ? (
              ...
              <button type="submit" className="w-full text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Reset</button>
            )}
          </form>
        </div>
      </div>
